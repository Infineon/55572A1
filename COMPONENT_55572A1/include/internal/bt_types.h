...
typedef struct
{
    ...
    UINT32      layer_flags;                    /* Layer-specific flags                 */
    void        *pv_app_ctx;                    /* App context                          */
    UINT8       *p_payload;                     /* Pointer to Payload                   */
    UINT16      payload_len;                    /* Payload length                       */
    UINT16      payload_offset;                 /* Payload offset                       */
    UINT16      prot_hdr_len;                   /* Protocol header length               */
    UINT16      prot_hdr_offset;                /* Protocol header offset               */
    UINT8       dtcb_flags;                     /* Flags as defined in DTCB_FLAGS_xxxxx */
    UINT8       rfcomm_crc;                     /* RFComm CRC (if indicated in flags)   */
    UINT8       protocol_hdr[1];                /* Protocol header (size varies)        */
} tDTCB;
