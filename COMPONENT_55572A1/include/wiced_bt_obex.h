...
#define OBEX_HDR_OFFSET             (18)

#define OBEX_MIN_MTU                255         /* per IrOBEX spec */
#define OBEX_MAX_TRANSPORT_MTU      1000        // TODO - make this more easily configurable
#define OBEX_RFC_FIFO_SIZE          (OBEX_MAX_TRANSPORT_MTU + OBEX_HDR_OFFSET)  // Allow for headers
#define OBEX_CMD_POOL_SIZE          256
#define OBEX_LRG_DATA_POOL_SIZE     1024        // TODO - make this more easily configurable
...
typedef struct
{
    ...
    wiced_bool_t                final;  /* WICED_TRUE, if this is the final packet of this PUT transaction. */
...
{
    uint8_t             ssn;        /* session sequence number */
    wiced_bool_t        final;      /* WICED_TRUE, if this is the final packet of this GET transaction. */
...
typedef struct
{
    ...
    wiced_bool_t                no_rsp;     /* WICED_TRUE, when the event is generated as a part of RESUME SESSION */
...
typedef union
{
    ...
    wiced_bt_obex_get_evt_t         get;        /* This element is associated with OBEX_GET_REQ_EVT. WICED_TRUE, if this is the final packet that contains the OBEX headers for this GET request. */
...
typedef struct
{
    ...
    uint16_t        transport_rx_mtu;
    uint16_t        obex_rx_mtu;
    ...
    wiced_bool_t    get_nonf;       /* report GET non-final request event. If WICED_FALSE, GET response is sent automatically */
    ...
    uint32_t        nonce;          /* This is converted to uint8_t[16] internally before adding to the OBEX header. This value is copied to the server control block and is increased after each use. 0, if only legacy OBEX (unreliable) session is desired. */
    wiced_bool_t    srm;            /* WICED_TRUE, to support single response mode. */
 ...
 *  @param[in]  mtu     : MTU
 *  @param[in]  srm     : WICED_TRUE - enable SRM, WICED_FALSE - no SRM
 ...
 *              function takes a GKI buffer and sets the offset in tOBEX_BUFFER as
 *              OBEX_HDR_OFFSET, the len as 0. The space_left is set to the
 *              length still available. This function compares the given
 *              (pkt_size - sizeof(uint8_t)) with the peer MTU to get the lesser
 *              of the two and set the space_left to
 ...
 *  @param[out]  p_end   : WICED_TRUE  : found a OBEX_HI_BODY_END header
 *                       : WICED_FALSE : found a OBEX_HI_BODY header or no body header was found
 *
