...
#define WICED_BLE_ISOC_MIN_TRANSPORT_LATENCY                                   \
  0x0005 /**< ISOC Minimum Latency                                             \
          */
#define WICED_BLE_ISOC_MAX_TRANSPORT_LATENCY                                   \
  0x0FA0 /**< ISOC Maximum Latency                                             \
          */

typedef enum {
  WICED_BLE_ISOC_DPD_INPUT =
      0, // ISO driver is source (Input data path (Host to Controller))
  WICED_BLE_ISOC_DPD_OUTPUT, // ISO driver is sink (Output data path (Controller
                             // to Host))
...
typedef enum {
  ...
  WICED_BLE_ISOC_DPD_INPUT_OUTPUT_BIT =
      WICED_BLE_ISOC_DPD_INPUT_BIT & WICED_BLE_ISOC_DPD_OUTPUT_BIT,
...
typedef uint8_t wiced_bt_isoc_packing_t; /**< ISOC packing methods (see
                                            #wiced_bt_isoc_packing_e) */
...
typedef uint8_t wiced_bt_isoc_framing_t; /**< ISOC Framing types (see
                                            #wiced_bt_isoc_framing_e) */
...
typedef uint8_t
    wiced_bt_isoc_phy_t; /**< ISOC LE PHY (see #wiced_bt_isoc_phy_e) */
...
typedef uint8_t wiced_bt_isoc_encryption_t; /**< ISOC Encryption (see
                                               #wiced_bt_isoc_encryption_e) */
...
typedef uint8_t
    wiced_bt_isoc_event_t; /**< ISOC Events (see #wiced_bt_isoc_event_e) */
...
typedef struct {
  uint8_t status; /**< CIG Establishment Status  (0 = Success). Refer Core Spec
                     v5.2 [Vol 1] Part F, Controller Error Codes */
  uint16_t cis_conn_handle; /**< CIS Connection Handle */
  uint8_t cig_id;           /**< CIG ID */
  uint8_t cis_id;           /**< CIS ID */
  uint32_t cig_sync_delay;  /**< CIG Sync Delay in microseconds */
  uint32_t cis_sync_delay;  /**< CIS Sync Delay in microseconds */
  uint32_t latency_c_to_p;  /**< Maximum time, in microseconds, for an SDU to be
                               transported from the master Controller to slave
                               Controller */
  uint32_t latency_p_to_c;  /**< Maximum time, in microseconds, for an SDU to be
                               transported from the slave Controller to master
                               Controller */
  wiced_bt_isoc_phy_t
      phy_c_to_p; /**< The transmitter PHY of packets from the master */
  wiced_bt_isoc_phy_t
      phy_p_to_c; /**< The transmitter PHY of packets from the slave */
  ...
  uint8_t ft_c_to_p; /**< Flush timeout, in multiples of the ISO_Interval for
                        master to slave transmission */
  uint8_t ft_p_to_c; /**< Flush timeout, in multiples of the ISO_Interval for
                        slave to master transmission */
  uint16_t max_pdu_c_to_p; /**< Maximum size, in bytes, of an SDU from the
                              master’s Host */
  uint16_t max_pdu_p_to_c; /**< Maximum size, in octets, of an SDU from the
                              slave’s Host */
...
typedef struct {
  uint8_t status; /**< SCA Status (0 = Success). Refer Core Spec v5.2 [Vol 1]
...
typedef struct {
  uint8_t status;    /**< Data path Status  (0 = Success). Refer Core Spec v5.2
                        [Vol 1] Part F, Controller Error Codes */
  uint16_t conn_hdl; /**< CIS/BIS Connection Handle  */
  wiced_bt_isoc_data_path_direction_t
      data_path_dir; /**< data path direction (valid for data path setup only)
                      */
...
typedef struct {
  ...
  uint8_t reason; /**< Reason for termination. Refer Core Spec v5.2 [Vol 1] Part
                     F, Controller Error Codes */
} wiced_bt_isoc_terminated_data_t;

/** ISOC BIG Sync Establishment data */
typedef struct {
  uint8_t status;     /**< Create BIG Status (0 = Success). Refer Core Spec v5.2
                         [Vol 1] Part F, Controller Error Codes */
  uint8_t big_handle; /**< BIG Handle */
  uint32_t trans_latency;      /**< The maximum delay time, in microseconds, for
                                  transmission of SDUs of all BISes in a BIG event */
  uint8_t number_of_subevents; /**< The number of subevents in each BIS event in
                                  the BIG */
  ...
  uint8_t immediate_repetition_count; /**< The number of times a payload is
                                         transmitted in a BIS event */
  uint16_t max_pdu;            /**< Maximum size, in octets, of the payload */
  uint16_t iso_interval;       /**< The time between two consecutive BIG anchor
                                  points. Time = N * 1.25 ms */
...
typedef union {
  ...
  wiced_bt_isoc_cis_established_data_t
      cis_established_data;                     /**< CIS Established */
  ...
  wiced_bt_isoc_data_path_status_t
      datapath; /**< Data Path Status (setup/remove) */
  wiced_bt_isoc_create_big_complete_t
      create_big; /**< Create BIG Command Status */
  wiced_bt_isoc_terminated_data_t
      terminate_big; /**< Terminate BIG Command Status */
  wiced_bt_isoc_big_sync_established_t
      big_sync_established; /**< BIG Sync Established data */
...
typedef struct {
  ...
  uint16_t max_sdu_c_to_p; /**< Maximum size, in bytes, of an SDU from the
                              master’s Host Valid Range 0x000 to 0xFFF*/
  uint16_t max_sdu_p_to_c; /**< Maximum size, in octets, of an SDU from the
                              slave’s Host Valid Range 0x000 to 0xFFF*/
  wiced_bt_isoc_phy_t
      phy_c_to_p; /**< The transmitter PHY of packets from the master */
  wiced_bt_isoc_phy_t
      phy_p_to_c;     /**< The transmitter PHY of packets from the slave */
  uint8_t rtn_c_to_p; /**< Maximum number of times every CIS Data PDU should be
                         retransmitted from the master to slave */
  uint8_t rtn_p_to_c; /**< Maximum number of times every CIS Data PDU should be
                         retransmitted from the slave to master */
} wiced_bt_ble_cis_config_t;

/** ISOC CIG Configuration */
typedef struct {
  uint8_t cig_id;               /**< CIG ID if known */
  uint32_t sdu_interval_c_to_p; /**< Time interval in microseconds between the
                                   start of consecutive SDUs from the master’s
                                   Host for all the CISes in the CIG */
  uint32_t sdu_interval_p_to_c; /**< Time interval in microseconds between the
                                   start of consecutive SDUs from the slave’s
                                   Host for all the CISes in the CIG. */
  uint8_t peripheral_clock_accuracy; /**< Slave Clock Accuracy */
  uint16_t max_trans_latency_c_to_p; /**< Maximum time, in microseconds, for an
                                        SDU to be transported from the master
                                        Controller to slave Controller */
  uint16_t max_trans_latency_p_to_c; /**< Maximum time, in microseconds, for an
                                        SDU to be transported from the slave
  ...
  uint8_t cis_count; /**< Total number of CISes in the CIG being added or
                        modified Valid Range 0x00 to 0x10 */
...
typedef struct {
  ...
  wiced_bt_isoc_phy_t
      phy_p_to_c;    /**< The transmitter PHY of packets from the slave */
  uint8_t bn_c_to_p; /**< Maximum number of times every CIS Data PDU should be
                        retransmitted from the master to slave */
  uint8_t bn_p_to_c; /**< Maximum number of times every CIS Data PDU should be
                        retransmitted from the slave to master */
...
typedef struct {
  ...
  uint32_t sdu_interval_p_to_c; /**< Time interval in microseconds between the
                                   start of consecutive SDUs from the slave’s
                                   Host for all the CISes in the CIG. */
  uint8_t ft_c_to_p;     /**< The flush timeout in multiples of ISO_Interval for
                            each payload sent from the master to slave. */
  uint8_t ft_p_to_c;     /**< The flush timeout in multiples of ISO_Interval for
                            each payload sent from the slave to master. */
 ...
 */
typedef void wiced_bt_isoc_cback_t(wiced_bt_isoc_event_t event,
 ...
 */
void wiced_bt_isoc_register_data_cb(
    wiced_bt_iso_rx_cb_t rx_data_cb,
 ...
 * @param[in]       ctx         : pointer to application data that will be given
 * back to application through the WICED_BLE_ISOC_SET_CIG_CMD_COMPLETE event
 *
 * @return      status
 *
 */
wiced_result_t
wiced_bt_isoc_central_set_cig_param(wiced_bt_ble_cig_param_t *cig_params,
                                    void *ctx);
 ...
 *
 */
wiced_result_t wiced_bt_isoc_central_set_cig_param_test(
    wiced_bt_ble_cig_param_test_t *cig_params, void *ctx);
 ...
 *
 * @param[in]       cis_count        : Number of elements in CIS handle list and
 * ACL handle list. These lists must be of the same length
 * @param[in]       cis_handle_list  : List of connection handles of the cis to
 * be established
 * @param[in]       acl_handle_list  : ACL connection handle associated with the
 * cis handles. The ith acl handle corresponds to the ith cis handle, etc.
 *
 * @return      status
 *
 *  Note : Once CIS is establish WICED_BLE_ISOC_CIS_ESTABLISHED event will be
 * received in registered application callback.
 */
wiced_result_t wiced_bt_isoc_central_create_cis(
    wiced_bt_isoc_create_cis_param_t *create_cis_param);
 ...
 *
 * @param[in]       cis_count         : Number of elements in Peer BDA list and
 * CIS handle list.
 ...
 *                  Slave should call this API on receiving
 * WICED_BLE_ISOC_CIS_REQUEST event in registered application callback
 *
 ...
 *                  Slave should call this API on receiving
 * WICED_BLE_ISOC_CIS_DISCONNECTED event in registered application callback and
 * if ASCS State is Releasing
 *
 ...
 * @return      status
 *
 *  Note : Once CIS is disconnected WICED_BLE_ISOC_CIS_DISCONNECTED event will
 * be received in registered application callback.
 */
 ...
 * @return      status
 *
 *  Note : WICED_BLE_ISOC_SLAVE_CLOCK_ACCURACY event will be received in
 * registered application callback.
 */
wiced_result_t
wiced_bt_isoc_central_request_peer_sca(wiced_bt_device_address_t peer_bda);
 ...
 */
wiced_result_t
wiced_bt_isoc_central_create_big(wiced_bt_isoc_create_big_param_t *p_big_param);
 ...
 */
wiced_bool_t wiced_bt_isoc_peripheral_big_create_sync(
    wiced_bt_isoc_big_create_sync_t *p_create_sync);
 ...
 */
wiced_bool_t wiced_bt_isoc_remove_data_path(
    uint16_t conn_hdl, wiced_bool_t is_cis,
 ...
 * @return wiced_bool_t
 */
wiced_bool_t wiced_bt_isoc_is_data_path_active(
 ...
 */
void wiced_bt_isoc_update_cis_conn_handle(uint8_t cig_id, uint8_t cis_id,
                                          uint16_t cis_conn_handle);
 ...
 * Supports only HCI for datapath and Does not support configuring codec in the
 * controller
 *
 * @param conn_hdl CIS/BIS Connection handle
 * @param is_cis TRUE if CIS connection handle is provided
 ...
 * @param controller_delay select a suitable Controller_Delay value from the
 * range of values provided by the HCI_Read_Local_Supported_Controller_Delay
 * command
 * @return wiced_bool_t TRUE if successful in sending the command
 */
wiced_bool_t
wiced_bt_isoc_setup_data_path(uint16_t conn_hdl, wiced_bool_t is_cis,
                              wiced_bt_isoc_data_path_direction_t data_path_dir,
