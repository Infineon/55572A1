...
typedef struct wiced_uart_transport_cfg_s
{
...
*/
typedef struct wiced_spi_transport_cfg_s
{
    uint32_t               clock_speed;         /**< Clock speed (non-zero for master, zero for slave).*/
    WICED_SPI_ENDIAN       endian;              /**< Direction of bit data flow (MSB or LSB first) */
    WICED_SPI_SS_POLARITY  polarity;            /**< Active high or active low for chip select line */
    WICED_SPI_MODE         mode;                /**< SPI mode (0-3). */
 ...
 * This function allocates a buffer internally from heap and prepare the header, copy the payload
 * and then sends the packet over the transport. Since it is allocating buffer from heap, there
 * is no size limit.
 *
 *@param[in]    code                  :Group code and command code
 ...
 */
wiced_result_t wiced_transport_send_hci_trace( wiced_bt_hci_trace_type_t type, uint8_t* p_data, uint16_t length );
